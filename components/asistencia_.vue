        ...
        <div class="row confirmation-section align-items-center">
            ...
            <div class="col-md-6" style="padding-right: 0px; padding-left: 0px;" :class="{ 'pases-section': search }">
                ...
                <div v-else >
                    ...
                    <p style="padding-left: 6%;"><strong>Selecciona la o las casillas de los nombres de los invitados que desees confirmar.</strong></p>
                    <form @submit.prevent="confirmarAsistencia" style="padding-left: 6%; margin-bottom: 12px;">
                        <div class="mb-3" v-for="(invitado, index) in invitados" :key="index">
                            <div class="form-check">
                                <input 
                                    class="form-check-input" 
                                    type="checkbox" 
                                    :id="`${invitado._id}`" 
                                    :value="invitado"
                                    v-model="confirmaciones"
                                >
                                <label class="form-check-label" :for="`invitado-${invitado._id}`">
                                    {{ invitado.name }}
                                </label>
                            </div>
                        </div>
  ...
  methods: {
    ...
    async confirmarAsistencia(){
        ...
        const resultado = this.invitados.map(invitado => {
            const estaConfirmado = this.confirmaciones.some(confirmacion => confirmacion._id === invitado._id);
            return {
                id: invitado._id, 
